...
#'
#' Converts a list object or several data.frames of lipid-level data to an object of the class 'lipidData'. Objects of the class 'lipidData' are lists with two obligatory components \code{e_data} and \code{f_data}. An optional list component \code{e_meta} is used if analysis or visualization at other levels (e.g. lipid identification) is also desired.
#'
#' @param e_data a \eqn{p \times n + 1} data.frame of expression data, where \eqn{p} is the number of lipids observed and \eqn{n} is the number of samples. Each row corresponds to data for each lipid. One column specifying a unique identifier for each lipid (row) must be present.
...
#' @param e_meta an optional data.frame with \eqn{p} rows. Each row corresponds to a lipid with one column giving lipid names (must be named the same as the column in \code{e_data}) and other columns giving meta information.
#' @param ... further arguments
#'
#' @details Objects of class 'lipidData' contain some attributes that are referenced by downstream functions. These attributes can be changed from their default value by manual specification. A list of these attributes as well as their default values are as follows:
#' \tabular{ll}{
#' edata_cname \tab Column name for lipid information found in \code{e_data} and \code{e_meta} (if applicable). Default value is 'Lipid'. \cr
#'  \tab \cr
#' emeta_cname \tab Column name for lipid information found in \code{e_meta} (if applicable). Default value is NULL. If \code{e_meta} is NULL, then \code{emeta_cname} will default to a NULL value. \cr
#'  \tab \cr
#' samp_cname \tab Column name for sample information found in \code{f_data}. Default value is 'sampleID'. \cr
#' \tab \cr
#' data_scale \tab Scale of the data provided in \code{e_data}. Acceptable values are 'log2', 'log10', 'log', and 'abundance', which indicate data is log base 2, base 10, natural log transformed, and raw abundance, respectively. Default values is 'log2'. \cr
#' \tab \cr
...
#' \tabular{ll}{
#' \code{num_edata} \tab The number of unique \code{edata_cname} entries.\cr
#' \tab \cr
#' \code{num_miss_obs} \tab The number of missing observations.\cr
#' \tab \cr
#' \code{num_emeta} \tab The number of unique \code{emeta_cname} entries. \cr
#' \tab \cr
#' \code{prop_missing} \tab The proportion of \code{e_data} values that are NA. \cr
#' \tab \cr
#' \code{num_samps} \tab The number of samples that make up the columns of \code{e_data}.\cr
#' \tab \cr
#' \code{meta_info} \tab A logical argument, specifying where the \code{e_meta} is provided.\cr
#' \tab \cr
#' }
#'
#' @examples
...
as.lipidData <- function(e_data, f_data, e_meta = NULL, ...){
  .as.lipidData(e_data, f_data, e_meta, ...)
}

## lipid data ##
.as.lipidData <- function(e_data, f_data, e_meta = NULL, edata_cname = "Lipid",
  ...
  if(!is.null(e_meta)){
    if(!(edata_cname %in% names(e_meta))) stop(paste("Lipid column ", edata_cname," not found in e_meta. Column names for lipid names must match for e_data and e_meta. See details of as.lipidData for specifying column names.", sep = ""))
  }
  ...
  if(!is.null(e_meta)){
    if(!is.null(emeta_cname)){
      if(!(emeta_cname %in% names(e_meta))) stop(paste("Identification column ", emeta_cname, " not found in e_meta. See details of as.lipidData for specifying column names.", sep = "") )
    }
  }
  
  # check that the Sample column name is in f_data column names #
  if(!(samp_cname %in% names(f_data))) stop(paste("Sample column ", samp_cname, " not found in f_data. See details of as.lipidData for specifying column names.", sep = ""))
  ...
  # if e_meta is provided, check that all lipids in e_data occur in e_meta #
  if(!is.null(e_meta)){
    if(sum(!(e_data[,edata_cname] %in% e_meta[,edata_cname])) > 0 ) stop("Not all lipids in e_data are present in e_meta")
  }
  ...
  if(!(data_scale %in% c('log2', 'log10', 'log', 'abundance'))) stop(paste(data_scale, " is not a valid option for 'data_scale'. See details of as.lipidData for specifics.", sep=""))
  ...
  if(!is.null(e_meta)){
    # number of lipid identifiers that map to a lipid in e_data #
    if(!is.null(emeta_cname)){
