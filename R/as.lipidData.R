...
#' @param edata_cname character string specifying the name of the column containing the lipid identifiers in \code{e_data} and \code{e_meta} (if applicable).
...
#'
#' @export
as.lipidData <- function(e_data, f_data, e_meta = NULL, edata_cname, fdata_cname, emeta_cname = NULL, ...){
...
.as.lipidData <- function(e_data, f_data, e_meta = NULL, edata_cname, fdata_cname,
  ...
  if(!(fdata_cname %in% names(f_data))) stop(paste("Sample column ", fdata_cname, " not found in f_data. See details of as.lipidData for specifying column names.", sep = ""))
  ...
  if(!is.null(e_meta)){
    if(any(!(e_meta[,edata_cname] %in% e_data[,edata_cname]))){
      ...
      warning("Extra lipids were found in e_meta that were not in e_data. These have been removed from e_meta.")
    }
  }
