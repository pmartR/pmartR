...
#' \tabular{ll}{
#' edata_cname  \tab Column name for protein information found in \code{e_data} and \code{e_meta} (if applicable). Default value is 'Protein'. \cr
#'  \tab \cr
#' emeta_cname  \tab Column name for gene information found in \code{e_meta} (if applicable). Default value is 'Gene'. If \code{e_meta} is NULL, then \code{emeta_cname} will default to a NULL value. \cr
#'  \tab \cr
...
as.proData <- function(e_data, f_data, e_meta = NULL, ...){
  .as.proData(e_data, f_data, e_meta, ...)
}
...
.as.proData <- function(e_data, f_data, e_meta = NULL, edata_cname = "Protein",
  ...
  if(!(samp_cname %in% names(f_data))) stop(paste("Sample column ", samp_cname, " not found in f_data. See details of as.proData for specifying column names.", sep = ""))
