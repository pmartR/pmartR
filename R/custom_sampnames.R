...
subset_names <- function(x, firstn, from, to, delim, components, pattern, ...) {
  if (!is.null(firstn)) {
    if (!is.null(from) |!is.null(to) | !is.null(delim) | !is.null(components) | 
        !is.null(pattern)) {
      stop("only firstn argument is needed")
    }
    
    if (!is.numeric(firstn)) {
      stop("firstn must be a non-negative numeric value less than the number ",
           "of characters in the smallest sample name")
    }

    output = lapply(x, function(x) {
      temp = strsplit(x, split = "")[[1]]
      
      if (firstn > length(temp)) {
        warning("there are less than ", firstn,
                " characters in a sample name, nothing was truncated")
      }
    ...
    if (!is.null(firstn) | !is.null(delim) | !is.null(components) |
        !is.null(pattern)) {
      stop("only from and to arguments are needed")
    }
    
    if (!is.numeric(from) | !is.numeric(to)) {
      stop("'from' and 'to' must be non-negative numeric values")
    }
    ...
    output = lapply(x, function(x) {
      ...
      if (!(from %in% 1:length(temp[[1]])) & !(to %in% 1:length(temp[[1]]))) {
        stop(from, " ", to, " are not in the range of the length of at least ", 
                   "one sample name")
      }
    ...
    if (!is.null(firstn) | !is.null(from) | !is.null(to) | !is.null(pattern)) {
      stop("only delim and components arguments are needed")
    } 

    output = lapply(x, function(x) {
      temp = strsplit(x, split = delim)[[1]]
      if (length(components) > length(temp)) {
        stop("the length of 'components vector must be less than ", 
             length(temp))
      }
      
      if (!any(components %in% 1:length(temp))) {
        stop("none of the indices specified in 'components' match indices of ",
             "the split sample name")
      }
