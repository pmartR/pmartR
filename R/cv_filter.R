...
cv_filter <- function(omicsData, use_groups = TRUE) {
  ...
  if (!inherits(omicsData,
                c("pepData", "proData", "lipidData", "metabData", "nmrData"))) {
    stop(
      "omicsData must be an object of class 'pepData', 'proData', 'lipidData', 'metabData', or 'nmrData'"
    )
  }
  ...
  if(use_groups == TRUE & !is.null(attr(omicsData, "group_DF"))) {
    ...
    temp_data2 <-
      temp_data[, match(as.character(groupDF[, samp_id]), names(temp_data))]
    ...
    pool_cv <-
      data.frame(mass_tag_id, CV_pooled, stringsAsFactors = FALSE)
    names(pool_cv)[1] <- edata_id
