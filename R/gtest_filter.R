...
gtest_filter <- function(nonmiss_per_group, mintR_groupDF=NULL, omicsData=NULL, e_data=NULL, alpha=NULL, min_nonmiss_gtest=NULL, cname_id = NULL, samp_id = NULL){

  # check that mintR_groupDF is supplied if e_data is supplied #
  if(!is.null(omicsData)){
    ...
    if(is.null(e_data)) stop("Either omicsData or e_data and mintR_groupDF must be provided.")
    if(is.null(mintR_groupDF)) stop("Either omicsData or e_data and mintR_groupDF must be provided.")
  ...
  if(!is.null(alpha)){
    ...
    group.vals.unique <- as.character(unique(mintR_groupDF$Group))
    ...
    for(i in 1:num.groups){
      ...
      group.size <- length(mintR_groupDF$Group[mintR_groupDF$Group==group.vals.unique[i]]) - length(mintR_groupDF$Group[mintR_groupDF$Group=="<NA>"])
      inds <- mintR_groupDF$Group==group.vals.unique[i] # which elements of mintR_groupDF are in the ith group?
      inds[is.na(inds)] <- FALSE
      cur.data <- rep(NA, length(mintR_groupDF$Sample.ID))
      cur.data[inds==TRUE] <- c(rep(1, group.size))
      cur.data <- t(data.frame(cur.data))
      if(mintR:::gtest_vector(cur.data, mintR_groupDF$Group)$pvalues > alpha){
        ...
        num.nonmiss <- 0 #max(table(mintR_groupDF$Group)) + 1000
        ...
        pval <- 1
        num.nonmiss <- 1
        while(pval > alpha && num.nonmiss < length(mintR_groupDF$Group[inds])){
          # modify cur.data #
          inds <- mintR_groupDF$Group==group.vals.unique[i]
          inds[is.na(inds)] <- FALSE # set any missing values to FALSE

          cur.data[inds] <- c(rep(1, num.nonmiss), rep(NA, length(mintR_groupDF$Group[inds])-num.nonmiss))
          pval <- mintR:::gtest_vector(cur.data, mintR_groupDF$Group)$pvalues
