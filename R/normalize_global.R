...
#' subset and normalization functions with possibility of apply the
...
#' @param params additional arguments passed to the chosen subset functions. See
#'   details for parameter specification and default values.
...
#'   (e.g. 'pepData') with normalized values in \code{e_data} is returned.
...
#'   for the proportion of features subset (rows of \code{e_data})
#'
...
#'   that includes all features (i.e. no subsetting is done). The option "los"
#'   identifies the subset of the features associated with the top \code{L} order statistics,
...
#'   features is taken as the subset identified (Wang et al., 2006). The option
...
#'   identifies features with complete data that have a p-value greater than a
...
#'   equivalent to "rip" however rather than requiring features with complete
#'   data, features with at least a \code{proportion} of non-missing values are
...
#'   estimates for mean centering. Parameters for z-score transformation are
...
#'   that must have non-missing values for a feature to be retained. Defaults to
...
#'   normalization parameters, number of features used in normalization, and
#'   proportion of features used in normalization. plot() and summary() methods
...
#' norm_object <- normalize_global(omicsData = lipid_object,
#'                                 subset_fn = "all",
#'                                 norm_fn = "median")
#' norm_data <- normalize_global(omicsData = lipid_object,
#'                               subset_fn = "all",
#'                               norm_fn = "median",
#'                               apply_norm = TRUE,
#'                               backtransform = TRUE)
