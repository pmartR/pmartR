...
.plot.naRes<- function(naRes_object, type, x_lab = NULL, y_lab = NULL, title_plot = NULL, title_size = 14, x_lab_size = 11, y_lab_size = 11, bar_width = .8, binwidth = 1, bw_theme = FALSE){
  ...
  if(type == "bySample"){
    ...
    if(bw_theme == FALSE){
      p<- ggplot2::ggplot(data=na.by.sample, aes(x=na.by.sample[[fdata_cname]], y = na.by.sample$num_NA)) + 
          ggplot2::geom_bar(stat = "identity", width = bar_width, color= "black", fill = "steelblue") +
    ...
    #plots NA per sample
    if(bw_theme == FALSE){
      s<- ggplot2::ggplot(data=na.by.sample, aes(x=na.by.sample[[fdata_cname]], y = na.by.sample$num_NA)) + 
          ggplot2::geom_bar(stat = "identity", width = bar_width, color= "black", fill = "steelblue") +
          ggplot2::geom_text(aes(label = num_NA), vjust = 2, color = "white", size = 3) +
