...
pquant_median <- function(pepData){
  ...
  res = as.data.frame(temp %>% dplyr::group_by_(pro_id) %>% dplyr::mutate_at(vars(-dplyr::group_cols()), median, na.rm = T))
  ...
  prodata = as.proData(e_data = data.frame(res, check.names=check_names), f_data = pepData$f_data, e_meta = e_meta, edata_cname = pro_id, fdata_cname = samp_id, emeta_cname = pro_id, data_scale = data_scale, is_normalized = is_normalized, check.names = check_names)
