...
data.frame(
  "Parameter Name" = c(
    "cognostics", "ggplot_params", "interactive", "path",
    "name", "test_mode", "test_example", "single_plot"
  ),
  "Description" = c(
    "Set the specific cognostics of the trelliscope. Varies per plotting function.",
...
Abun_Box_Sample <- trelli_abundance_boxplot(SampleGroups,
  include_points = F, single_plot = T,
...
lipids <- normalize_global(lipids,
  subset_fn = "rip", norm_fn = "median",
...
trelliData3 %>%
  ...
  trelli_foldchange_bar(
    p_value_thresh = 0.05, p_value_test = TRUE,
...
data.frame(
  Function = c(
    "xlab('Name')", "ylab('Name')", "ggtitle('Name')",
  ...
  Purpose = c(
    "Rename x-axis label", "Rename y-axis label", "Rename plot",
...
data.frame(
  Functions = c(
    "trelli_abundance_boxplot", "trelli_abundance_histogram",
