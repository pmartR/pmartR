...
test_that('normalize_quantile produces the correct output',{
  ...
  expect_identical(attr(a_norm, "cnames"),
                   attr(nmrdata, "cname"))
  expect_identical(attr(a_norm, "nmr_info"),
                   attr(nmrdata, "nmr_info"))
  expect_identical(attr(a_norm, "meta_info"),
                   attr(nmrdata, "meta_info"))
  expect_identical(attr(a_norm, "filters"),
                   attr(nmrdata, "filters"))
  expect_equal(attributes(a_norm)$data_info,
  ...
  # Check out the attributes.
  expect_identical(attr(l_norm, "cnames"),
                   attr(lognmrdata, "cname"))
  expect_identical(attr(l_norm, "nmr_info"),
                   attr(lognmrdata, "nmr_info"))
  expect_identical(attr(l_norm, "meta_info"),
                   attr(lognmrdata, "meta_info"))
  expect_identical(attr(l_norm, "filters"),
                   attr(lognmrdata, "filters"))
  expect_equal(attributes(l_norm)$data_info,
               list(data_scale_orig = "abundance",
                    data_scale = "log",
                    norm_info = list(is_normalized = TRUE,
                                     norm_type = "quantile"),
  ...
  expect_error(normalize_quantile(omicsData = ldata),
