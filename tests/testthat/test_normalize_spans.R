...
test_that('SPANS correctly spans the data',{
  ...
  group <- c(rep("Infection", 9),
             rep("Mock", 3))
  group_eggs <- c("Infection", "Mock", "Mock", "Infection", "Infection",
  ...
  true_sig_inds <- c(2, 6, 8, 9, 10, 14, 15, 24, 26, 32, 40, 41, 44, 47, 48, 66,
                     67, 75, 78, 81, 85, 87, 93, 102, 103, 104, 105, 109, 112,
                     113, 115, 117, 129, 131, 147)
  sig_inds[true_sig_inds] <- TRUE
  
  nonsig_inds <- rep(FALSE, 150)
  true_nonsig_inds <- c(18, 33, 34, 52, 56, 59, 80, 83, 88, 89, 91, 92, 98, 119,
                        122, 123, 127, 136, 142, 143, 146)
  nonsig_inds[true_nonsig_inds] <- TRUE
  
  select_n <- c(31, 248, 1363, 875, 1242, 1341, 344, 67, 1321, 681, 1194, 1446,
  ...
  spandard <- data.frame(
    subset_method = c("rip", "ppp_rip", "rip", "ppp_rip", "rip", "ppp_rip",
                      "rip", "all", "ppp_rip", "all", "all", "all", "los",
                      "ppp", "ppp", "ppp", "ppp", "los", "los", "los"),
    normaliztion_method = c("median", "median", "mean", "mean", "zscore",
                            "zscore", "mad", "mean", "mad", "zscore", "mad",
                            "median", "mean", "mean", "zscore", "median",
                            "mad", "median", "zscore", "mad"),
    SPANS_score = c(0.87, 0.87, 0.87, 0.87, 0.8, 0.8, 0.8, 0.78, 0.78, 0.7, 0.7,
                    0.66, 0.55, 0.55, 0.55, 0.53, 0.53, NA, NA, NA),
    parameters = c("0.2", "0.5;0.2", "0.2", "0.5;0.2", "0.2", "0.5;0.2",
                   "0.2", "", "0.5;0.2", "", "", "", "0.05", "0.5", "0.5",
                   "0.5", "0.5", "0.05", "0.05", "0.05"),
    mols_used_in_norm = c(22, 27, 22, 27, 22, 27, 22, 150, 27, 150, 150, 150,
                          14, 124, 124, 124, 124, 14, 14, 14),
    passed_selection = c(rep(TRUE, 17),
                         rep(FALSE, 3))
  )
  
  # Set up the attributes for the spandard :)
  attributes(spandard) <- list(
    names = c("subset_method", "normalization_method", "SPANS_score",
    ...
    method_selection_pvals = data.frame(
      ...
      location_p_value = c(0.926340489251137, 0.926340489251137,
                           ...
                           0.0522036353413146, 0.020819004906484),
      scale_p_value = c(NA, NA, NA, NA, 0.405380556458942, 0.517534719643234,
      ...
      F_log_HSmPV = c(rep(1, 17),
                      rep(NA, 3)),
      F_log_NSmPV = c(0.74, 0.74, 0.74, 0.74, 0.6, 0.6, 0.6, 0.56, 0.56, 0.4,
