...
test_that('bpquant produces correct isoforms',{
  ...
  expect_equal(dim(attr(bayes, "isoformRes_subset")),
               c(98, 3))
  expect_equal(bayes[[1]],
               data.frame(Protein = rep("ALBU_HUMAN", 5),
                          Mass_Tag_ID = c("1047", "1104", "1237", "1768",
                                          "4198254"),
                          proteoformID = c(0, 1, 1, 1, 1)))
  expect_equal(bayes[[5]],
               data.frame(Protein = rep("PYGB_HUMAN", 5),
                          Mass_Tag_ID = c("1649", "11055", "11078", "12404",
                                          "21149"),
                          proteoformID = c(1, 1, 0, 1, 1)))
  expect_equal(bayes[[22]],
               data.frame(Protein = "EFTU_HUMAN",
                          Mass_Tag_ID = c("6880071"),
                          proteoformID = c(1)))
  expect_equal(bayes[[36]],
               data.frame(Protein = "XRCC5_HUMAN",
                          Mass_Tag_ID = c("6948834"),
                          proteoformID = c(1)))
  expect_equal(bayes[[63]],
               data.frame(Protein = "HNRPK_HUMAN",
                          Mass_Tag_ID = c("6948906"),
                          proteoformID = c(1)))
  expect_equal(attr(bayes, "isoformRes_subset")$Protein,
               prots)
  expect_equal(attr(bayes, "isoformRes_subset")$Protein_Isoform,
               prots)
  ...
  expect_equal(round(bayes_mod$post_prob, 5),
               round(c(4.827795e-04, 8.469562e-01, 8.691286e-05, 1.524741e-01),
                     5))
  expect_equal(bayes_mod$peptide_idx,
               c(0, 1, 1, 1, 1))
  expect_equal(bayes_mod$unique_sigs$flags1,
               c(-1, 1))
  expect_equal(bayes_mod$num_proteoforms,
               1)
  expect_equal(bayes_mod$proteoform_configs,
               matrix(c(0, 0,
                        1, 0,
                        0, 1,
                        1, 1),
  ...
  expect_identical(
    iso_fun,
    data.frame(Protein = rep("ALBU_HUMAN", 4),
  ...
  expect_identical(
    iso_fun,
    data.frame(Protein = rep("6PGL_HUMAN", 5),
               Protein_Isoform = c("6PGL_HUMAN;1", "6PGL_HUMAN;1",
