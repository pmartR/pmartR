...
The comparison of samples at two points from the same individual (e.g., a before and after state) is common in omics experiments and referred to as “paired data.” Functions in `pmartR` impacted by the use of paired data have been updated to properly handle paired data throughout the package.
...
The $p$-values associated with each group comparison can be adjusted for multiple comparisons using the `pval_adjust_g` argument to `imd_anova`. 
