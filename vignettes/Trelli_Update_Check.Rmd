...
Requires statRes and paneling by an e_meta column

```{r}
# Should return biomolecule count, proportion significant, mean fold change,
# and sd fold change
trelli_panel_by(trelliData = trelliData4, panel = "RazorProtein") %>%
  trelli_foldchange_boxplot(test_mode = TRUE, test_example = 1:10,
                            cognostics = c("biomolecule count", "proportion significant",
                                           "mean fold change", "sd fold change"))

# Should return biomolecule count and proportion significant
trelli_panel_by(trelliData = trelliData4, panel = "RazorProtein") %>%
  trelli_foldchange_boxplot(test_mode = TRUE, test_example = 1:10, cognostics = c("biomolecule count", "proportion significant"))
```
...
# and sd fold change
trelli_panel_by(trelliData = trelliData4, panel = "RazorProtein") %>%
  trelli_foldchange_heatmap(test_mode = TRUE, test_example = 1:10,
                            cognostics = c("biomolecule count", "proportion significant",
                                           "mean fold change", "sd fold change"))

# Should return biomolecule count and proportion significant
trelli_panel_by(trelliData = trelliData4, panel = "RazorProtein") %>%
  trelli_foldchange_heatmap(test_mode = TRUE, test_example = 1:10, cognostics = c("biomolecule count", "proportion significant"))
```

## Foldchange Volcano

```{r}
# Should return biomolecule count, proportion significant, proportion significant 
# up, and proportion significant down 
trelli_panel_by(trelliData = trelliData4, panel = "RazorProtein") %>%
  trelli_foldchange_volcano(test_mode = TRUE, test_example = 1:10, 
                            comparison = "Phenotype3_vs_Phenotype1",
                            cognostics = c("biomolecule count", "proportion significant",
                                           "proportion significant up", "proportion significant down"))


# Should return proportion significant
trelli_panel_by(trelliData = trelliData4, panel = "RazorProtein") %>%
  trelli_foldchange_volcano(test_mode = TRUE, test_example = 1:10, 
                            comparison = "Phenotype3_vs_Phenotype1",
                            cognostics = "proportion significant")

# Should return proportion significant and comparison
trelli_panel_by(trelliData = trelliData4, panel = "RazorProtein") %>%
  trelli_foldchange_volcano(test_mode = TRUE, test_example = 1:10, 
                            comparison = c("Phenotype3_vs_Phenotype1",
                                           "Phenotype3_vs_Phenotype2"),
                            cognostics = "proportion significant")

# If no comparisons are specified, then all of them should be generated
trelli_panel_by(trelliData = trelliData4, panel = "RazorProtein") %>%
  trelli_foldchange_volcano(test_mode = TRUE, test_example = 1:10, 
                            comparison = "all",
