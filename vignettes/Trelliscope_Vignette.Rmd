...
```{r, echo = F}
datatable(head(pmartRdata::lipid_edata_pos), options = list(`scrollX` = T))
```
...
which include the edata_cname (in this case, "LipidCommonName"), and a "Sample" category which 
...
```{r}
summary(trelliData1 %>% trelli_panel_by("LipidCommonName")) 
```
...
a "LipidCommonName" example and on the right, a "Sample" example, with the color changed
to avoid confusion. 
...
by edata_cname ("LipidCommonName")...
...
...or e_meta variable. In this case, our options include LipidFamily. 
