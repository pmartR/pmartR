...
search, sort, and filter plots based on **congostics**, which include single data 
...
trelli_abundance_boxplot(PeptideGroups, single_plot = TRUE, test_example = 3)

# Make an abundance boxplot without the points 
trelli_abundance_boxplot(SampleGroups, include_points = F, single_plot = T,
...
#### What do I do with my transcriptomics data (seqData)? 

If plotting "abundances" or "missingness", different functions are used when plotting
seqData, though the "fold change" plots are the same. Transcript counts are log
counts per million transformed when plotting. Oftentimes, these trelliscope displays
are quite large and need additional filtering. 

| MS/NMR Plot Name | seqData Equivalent |
|------------------|--------------------|
| abundance_boxplot | rnaseq_boxplot |
| abundance_histogram | rnaseq_histogram | 
| abundance_heatmap | rnaseq_heatmap |
| missingness_bar | rnaseq_nonzero_bar |
