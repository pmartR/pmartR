...
The pmartRqc package contains four methods for protein quantitation, rollup, rrollup, qrollup and zrollup. These functions are applied within a wrapper function, `protein_quant`. The inputs of `protein_quant` are, a pepData object, a method (one of 'rollup', 'rollup', 'qrollup' or 'zrollup'),an optional isoformRes object which defaults to NULL and 'combine_fn'(either mean or median). In the case where 'isoformRes' is left NULL, depending on the input for 'method', the correct protein quantitation method is applied to the input pepData object and a proData object is returned. 

#### Rollup Methods

In the rrollup method, all peptides that map to a single protein are scaled based on a chosen reference peptide, which is the peptide with most observations. Next the average or mean of the scaled peptides is set as the relative protein abundance.
