...
The pmartRqc package contains various methods for data summarization and exploration that can be used as part of the quality control process: numeric summaries and associated plots (via the `summarize()` function), sequential projection pursuit principal components analysis (sppPCA) for dimension reduction (via the`dim_reduction()` function), and a correlation heat map (via the `cor_result()` function). As for missing data summarization there are also functions to collect and plot missing data information. Missing data collection and plotting functions include, `missingval_result()`, `plot_missingval()`, `missingval_scatterplot()` and `missingval_heatmap()`. 
...
Laurent, G., Leslie C., Benjamin, M. B., Rafael, A. I. (2004). affy-analysis of Affymetrix GeneChip data at the probe level. Bioinformatics. 20(3): 307-315.
...
Strand, M. (2003). Contrast normalization of oligonucleotide arrays. Journal of Computational Biology. 10(1): 95-102. 
