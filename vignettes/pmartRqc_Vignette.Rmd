...
rm(mypepData_temp)
```
Here we keep sample `Infection 1`, `Infection 2`, `Infection 3`, `Infection 4` and `Infection 5`
```{r custom_filter2}
...
The pmartRqc package contains two methods for data summarization and exploration that can be used as part of the quality control process: sequential projection pursuit principal components analysis (sppPCA) for dimension reduction and a correlation heat map. As for missing data summarization there are also functions to collect and plot missing data information. Missing data collection and plotting functions include, `missingval_result`, `plot_missingval`, `missingval_scatterplot` and `missingval_heatmap`. 

#### Probabilistic Principal Components Analysis

Probabilistic is a PCA algorithm that can be used in the presence of missing data. We use the `dim_reduction()` function in pmartRqc, specifying the dataset and the number of principal components to return (should we make this default to 2?). The `summary()` and `plot()` functions operate on the results of the `dim_reduction()` function.
...
#### Missingval_result and plot_missingval functions
...
#### Missingval_scatterplot and missingval_heatmap functions
...
The pmartRqc package contains five methods for protein quantitation, pquant_mean, pquant_median, rrollup, qrollup and zrollup. These functions are applied within a wrapper function, `protein_quant`. The inputs of `protein_quant` are, a pepData object, a method (one of 'mean', 'median' or 'rrollup') and an optional isoformRes object which defaults to NULL. In the case where 'isoformRes' is left NULL, depending on the input for 'method', the correct protein quantitation method is applied to the input pepData object and a proData object is returned. 

#### Rollup Methods

In the rrollup method, all peptides that map to a single protein are first scaled based on a chosen reference peptide, which is the peptide with most observations. Next the average or mean of the scaled peptides is set as the relative protein abundance.
The qrollup method starts with all peptides that map to a single protein. Next peptides are selected according to a user selected abundance cutoff value and the average or mean of the scaled peptides is set as the protein abundance.
In the zrollup method, scaling is done similarly to the  z-score formula (except with medians instead of means). The scaling formula is applied to peptides that map to a single protein and then mean or median of the scaled peptides is set as protein abundance. 

#### Pquant Mean and Median

The pquant_mean and pquant_median methods are similar to the rollup methods, except there is no scaling involved in these methods. Either the mean or median is applied to all peptides that map to a single protein to obtain protein abundance.


```{r protein_quant}
print(mypepData)

mypepdata_median<- protein_quant(pepData = mypepData, method = "median")
