...
The pmartRqc package contains three functions for protein quantitation, `pquant_mean`, `pquant_median`, and `rrollup`. These functions are applied within a wrapper function, `protein_quant`. The inputs of `protein_quant` are, a pepData object, a method (one of 'mean', 'median' or 'rrollup') and an optional isoformRes object which defaults to NULL. In the case where 'isoformRes' is left NULL, depending on the input for 'method', the correct protein quantitation function is applied to the input pepData object and a proData object is returned. 

¿FURTHER DESCRIPTION OF EACH PROTEIN QUANTITATION METHOD?
